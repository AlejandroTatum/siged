...
export function PlanningPage() {
  ...
  const load=useCallback(async()=>{ if(!token||access!=="allowed")return; setError(""); try {
    ...
    if(section==="asignaturas"&&parentId){const [d,grade]=await Promise.all([planningApi.subjects(token,parentId),planningApi.grade(token,parentId)]);setItems(d);setGradeAlert(grade);}
  ...
  async function submit(e:React.FormEvent){e.preventDefault();if(!token)return;setBusy(true);setError("");setNotice("");try{
    ...
    const data=section==="planes"?{nombre:name,es_activo:active}:section==="grados"?{nombre:name,orden:order,nivel:Number(levelId),subnivel:sublevelId?Number(sublevelId):null}:{nombre:name,pp_semana_minimo:Number(weeklyLoad)};
  ...
  function edit(x:Item){setEditingId(x.id);setName(x.nombre);if("es_activo" in x)setActive(x.es_activo);if("nivel" in x){setLevelId(String(x.nivel.id));setSublevelId(String(x.subnivel?.id??""))}if("pp_semana_minimo" in x)setWeeklyLoad(String(x.pp_semana_minimo))}
  ...
  return <main className="space-y-6 p-8"><header className="border border-t-4 border-t-primary bg-heading-block p-6"><h1 className="text-3xl font-bold">{heading}</h1></header>
    ...
    {gradeAlert?.alerta_carga_pedagogica&&<p role="alert">Load alert: {gradeAlert.carga_pedagogica_actual} / {gradeAlert.carga_pedagogica_minima}</p>}
  </main>
}
